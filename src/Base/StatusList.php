...
abstract class StatusList {
    ...
    const AUTO_MOD_TRIGGER_TYPE_KEYWORD = 1;
    const AUTO_MOD_TRIGGER_TYPE_SPAM = 3;
    const AUTO_MOD_TRIGGER_TYPE_KEYWORD_PRESET = 4;
    const AUTO_MOD_TRIGGER_TYPE_MENTION_SPAM = 5;

    const AUTO_MOD_KEYWORD_PRESET_TYPE_PROFANY = 1;
    const AUTO_MOD_KEYWORD_PRESET_TYPE_SEXUAL_CONTENT = 2;
    const AUTO_MOD_KEYWORD_PRESET_TYPE_SLURS = 3;

    const AUTO_MOD_EVENT_TYPE_MESSAGE_SEND = 1;

    const AUTO_MOD_ACTION_TYPE_BLOCK_MESSAGE = 1;
    const AUTO_MOD_ACTION_TYPE_SEND_ALERT_MESSAGE = 2;
    const AUTO_MOD_ACTION_TYPE_TIMEOUT = 3;
    ...
    const CHANNEL_TYPE_GUILD_DIRECTORY = 14;
    const CHANNEL_TYPE_GUILD_FORUM = 15;
    const CHANNEL_TYPE_GUILD_MEDIA = 16;

    const CHANNEL_VIDEO_QUALITY_AUTO = 1;
    const CHANNEL_VIDEO_QUALITY_FULL = 2;

    const CHANNEL_FLAG_PINNED = 1 << 1;
    const CHANNEL_FLAG_REQUIRE_TAG = 1 << 4;
    const CHANNEL_FLAG_HIDE_MEDIA_DOWNLOAD_OPTIONS = 1 << 15;

    const CHANNEL_SORT_ORDER_LATEST_ACTIVITY = 0;
    const CHANNEL_SORT_ORDER_CREATION_DATE = 1;

    const CHANNEL_FORUM_LAYOUT_TYPES_NOT_SET = 0;
    const CHANNEL_FORUM_LAYOUT_TYPES_LIST_VIEW = 1;
    const CHANNEL_FORUM_LAYOUT_TYPES_GALLERY_VIEW = 2;
    ...
    const GUILD_SYSCHANNEL_FLAG_SUPPRESS_JOIN_NOTIFICATION_REPLIES = 1 << 3;
    const GUILD_SYSCHANNEL_FLAG_SUPPRESS_ROLE_SUBSCRIPTION_PURCHASE_NOTIFICATIONS = 1 << 4;
    const GUILD_SYSCHANNEL_FLAG_SUPPRESS_ROLE_SUBSCRIPTION_PURCHASE_NOTIFICATION_REPLIES = 1 << 5;

    const GUILD_FEATURE_ANIMATED_BANNER = 'ANIMATED_BANNER';
    const GUILD_FEATURE_ANIMATED_ICON = 'ANIMATED_ICON';
    const GUILD_FEATURE_APPLICATION_COMMAND_PERMISSIONS_V2 = 'APPLICATION_COMMAND_PERMISSIONS_V2';
    const GUILD_FEATURE_AUTO_MODERATION = 'AUTO_MODERATION';
    const GUILD_FEATURE_BANNER = 'BANNER';
    const GUILD_FEATURE_COMMUNITY = 'BANNER';
    const GUILD_FEATURE_CREATOR_MONETIZABLE_PROVISIONAL = 'CREATOR_MONETIZABLE_PROVISIONAL';
    const GUILD_FEATURE_CREATOR_STORE_PAGE = 'CREATOR_STORE_PAGE';
    const GUILD_FEATURE_DEVELOPER_SUPPORT_SERVER = 'DEVELOPER_SUPPORT_SERVER';
    const GUILD_FEATURE_DISCOVERABLE = 'DISCOVERABLE';
    const GUILD_FEATURE_FEATURABLE = 'FEATURABLE';
    const GUILD_FEATURE_INVITES_DISABLED = 'INVITES_DISABLED';
    const GUILD_FEATURE_INVITE_SPLASH = 'INVITE_SPLASH';
    const GUILD_FEATURE_MEMBER_VERIFICATION_GATE_ENABLED = 'MEMBER_VERIFICATION_GATE_ENABLED';
    const GUILD_FEATURE_MORE_STICKERS = 'MORE_STICKERS';
    const GUILD_FEATURE_NEWS = 'NEWS';
    const GUILD_FEATURE_PARTNERED = 'PARTNERED';
    const GUILD_FEATURE_PREVIEW_ENABLED = 'PREVIEW_ENABLED';
    const GUILD_FEATURE_RAID_ALERTS_DISABLED = 'RAID_ALERTS_DISABLED';
    const GUILD_FEATURE_ROLE_ICONS = 'ROLE_ICONS';
    const GUILD_FEATURE_ROLE_SUBSCRIPTIONS_AVAILABLE_FOR_PURCHASE = 'ROLE_SUBSCRIPTIONS_AVAILABLE_FOR_PURCHASE';
    const GUILD_FEATURE_ROLE_SUBSCRIPTIONS_ENABLED = 'ROLE_SUBSCRIPTIONS_ENABLED';
    const GIULD_TICKETED_EVENTS_ENABLED = 'TICKETED_EVENTS_ENABLED';
    const GUILD_FEATURE_VANITY_URL = 'VANITY_URL';
    const GUILD_FEATURE_VERIFIED = 'VERIFIED';
    const GUILD_FEATURE_VIP_REGIONS = 'VIP_REGIONS';
    const GUILD_FEATURE_WELCOME_SCREEN_ENABLED = 'WELCOME_SCREEN_ENABLED';

    const GUILD_MUTABLE_FEATURE_COMMUNITY = 'COMMUNITY';
    const GUILD_MUTABLE_FEATURE_DISCOVERABLE = 'DISCOVERABLE';
    const GUILD_MUTABLE_FEATURE_INVITES_DISABLED = 'INVITES_DISABLED';
    const GUILD_MUTABLE_FEATURE_RAID_ALERTS_DISABLED = 'RAID_ALERTS_DISABLED';

    const GUILD_SCHEDULED_EVENT_PRIVACY_TYPE_STAGE_INSTANCE = '1';
    const GUILD_SCHEDULED_EVENT_PRIVACY_TYPE_VOICE = '2';
    const GUILD_SCHEDULED_EVENT_PRIVACY_TYPE_EXTERNAL = '3';

    const STAGE_INSTANCE_PUBLIC = 1;
    const STAGE_INSTANCE_GUILD_ONLY = 2;

    const STICKER_TYPE_STANDART = 1;
    ...
    const USER_FLAG_BOT_HTTP_INTERACTIONS = 1 << 19;
    const USER_FLAG_ACTIVE_DEVELOPER = 1 << 22;
    ...
    const AUDIT_LOG_EVT_APPLICATION_COMMAND_PERMISSION_UPDATE = 121;
    const AUDIT_LOG_EVT_AUTO_MODERATION_RULE_CREATE = 140;
    const AUDIT_LOG_EVT_AUTO_MODERATION_RULE_UPDATE = 141;
    const AUDIT_LOG_EVT_AUTO_MODERATION_RULE_DELETE = 142;
    const AUDIT_LOG_EVT_AUTO_MODERATION_BLOCK_MESSAGE = 143;
    const AUDIT_LOG_EVT_AUTO_MODERATION_FLAG_TO_CHANNEL = 144;
    const AUDIT_LOG_EVT_AUTO_MODERATION_USER_COMMUNICATION_DISABLED = 145;
    const AUDIT_LOG_EVT_CREATOR_MONETIZATION_REQUEST_CREATED = 150;
    const AUDIT_LOG_EVT_CREATOR_MONETIZATION_TERMS_ACCEPTED = 151;
    ...
    const APPLICATION_FLAG_APPLICATION_AUTO_MODERATION_RULE_CREATE_BADGE = 1 << 6;
    ...
    const APPLICATION_FLAG_VERIFICATION_PENDING_GUILD_LIMIT = 1 << 16;
    const APPLICATION_FLAG_EMBEDDED = 1 << 17;
    const APPLICATION_FLAG_GATEWAY_MESSAGE_CONTENT = 1 << 18;
    const APPLICATION_FLAG_GATEWAY_MESSAGE_CONTENT_LIMITED = 1 << 19;
    const APPLICATION_FLAG_APPLICATION_COMMAND_BADGE = 1 << 23;

    const APPLICATION_ROLE_CONNECTION_META_TYPE_INTEGER_LESS_THAN_OR_EQUAL = 1;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_INTEGER_GREATER_THAN_OR_EQUAL = 2;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_INTEGER_EQUAL = 3;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_INTEGER_NOT_EQUAL = 4;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_DATETIME_LESS_THAN_OR_EQUAL = 5;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_DATETIME_GREATER_THAN_OR_EQUAL = 6;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_BOOLEAN_EQUAL = 7;
    const APPLICATION_ROLE_CONNECTION_META_TYPE_BOOLEAN_NOT_EQUAL = 8;
    ...
    const PERMISSION_CREATE_INSTANT_INVITE = 1 << 0;
    const PERMISSION_KICK_MEMBERS = 1 << 1;
    const PERMISSION_BAN_MEMBERS = 1 << 2;
    const PERMISSION_ADMINISTRATOR = 1 << 3;
    const PERMISSION_MANAGE_CHANNEL = 1 << 4;
    const PERMISSION_MANAGE_GUILD = 1 << 5;
    const PERMISSION_ADD_REACTIONS = 1 << 6;
    const PERMISSION_VIEW_AUDIT_LOG = 1 << 7;
    const PERMISSION_PRIORITY_SPEAKER = 1 << 8;
    const PERMISSION_STREAM = 1 << 9;
    const PERMISSION_VIEW_CHANNEL = 1 << 10;
    const PERMISSION_SEND_MESSAGES = 1 << 11;
    const PERMISSION_SEND_TTS_MESSAGES = 1 << 12;
    const PERMISSION_MANAGE_MESSAGES = 1 << 13;
    const PERMISSION_EMBED_LINKS = 1 << 14;
    const PERMISSION_ATTACH_FILES = 1 << 15;
    const PERMISSION_READ_MESSAGE_HISTORY = 1 << 16;
    const PERMISSION_MENTION_EVERYONE = 1 << 17;
    const PERMISSION_USE_EXTERNAL_EMOJIS = 1 << 18;
    const PERMISSION_VIEW_GUILD_INSIGHTS = 1 << 19;
    const PERMISSION_CONNECT = 1 << 20;
    const PERMISSION_SPEAK = 1 << 21;
    const PERMISSION_MUTE_MEMBERS = 1 << 22;
    const PERMISSION_DEAFEN_MEMBERS = 1 << 23;
    const PERMISSION_MOVE_MEMBERS = 1 << 24;
    const PERMISSION_USE_VAD = 1 << 25;
    const PERMISSION_CHANGE_NICKNAME = 1 << 26;
    const PERMISSION_MANAGE_NICKNAMES = 1 << 27;
    const PERMISSION_MANAGE_ROLES = 1 << 28;
    const PERMISSION_MANAGE_WEBHOOKS = 1 << 29;
    const PERMISSION_MANAGE_GUILD_EXPRESSIONS = 1 << 30;
    const PERMISSION_USE_APPLICATION_COMMANDS =  1 << 31;
    const PERMISSION_REQUEST_TO_SPEAK = 1 << 32;
    const PERMISSION_MANAGE_EVENTS = 1 << 33;
    const PERMISSION_MANAGE_THREADS = 1 << 34;
    const PERMISSION_CREATE_PUBLIC_THREADS = 1 << 35;
    const PERMISSION_CREATE_PRIVATE_THREADS = 1 << 36;
    const PERMISSION_USE_EXTERNAL_STICKERS = 1 << 37;
    const PERMISSION_SEND_MESSAGES_IN_THREADS = 1 << 38;
    const PERMISSION_USE_EMBEDDED_ACTIVITIES = 1 << 39;
    const PERMISSION_MODERATE_MESSAGES = 1 << 40;
    const PERMISSION_VIEW_CREATOR_MONETIZATION_ANALYTICS = 1 << 41;
    const PERMISSION_USE_SOUNDBOARD = 1 << 42;
    const PERMISSION_USE_EXTERNAL_SOUNDS = 1 << 45;
    const PERMISSION_SEND_VOICE_MESSAGES = 1 << 46;
